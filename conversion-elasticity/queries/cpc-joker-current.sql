/*
    ...
    The filter for campaigns not yet ended is in the campaigns_dataset CTE (the first one)
        `DATE(ended_at_utc) > CURRENT_DATE`
...
DECLARE ENTITIES ARRAY<STRING> DEFAULT ['TB_BH', 'TB_AE'];
...
campaigns_dataset AS (
  ...
  WHERE global_entity_id IN  UNNEST(ENTITIES)
    -- filter only for those not yet ended
